...
workspace `default` → 引擎内置默认。`yzrws workitem start` 启动时按回退链加载
...
> **报错**；`yzrws workitem start` 在临时 `--engine` 切换到不兼容 engine 时仅 **WARN**。
...
```sh
# 创建 workitem 时指定引擎
yzrws workitem create my-task --engine opencode

# 运行时临时切换引擎（不写回 setting.json）
yzrws workitem start my-task --engine opencode
```
...
# 恢复（继续上次会话，未实现）
# yzrws workitem start <workitem_name> --resume

# 指定引擎（覆盖 setting.json；当前 --engine 仅作用于自动创建 / 切换时的初始 engine）
yzrws workitem start <workitem_name> --engine opencode
```
