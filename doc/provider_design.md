...
- `yzrws workitem start` 临时通过 `--engine` 切换到不兼容的 engine 时仅打印 **WARN**，不阻止启动
  （允许用户显式尝试）
...
> **本回退链由 `yzrws workitem set-model` / `yzrws workitem start` 实现消费**：
> `yzrws workitem set-model` 把 Provider 引用写入 `setting.json.provider`；
> `yzrws workitem start` 在启动前调用 `provider.resolve_model_config` 解析生效配置，
> 并把 `(base_url, auth_key, model)` 注入到引擎进程（见
> [`multi_agent_design.md`](./multi_agent_design.md)）。

当 `yzrws workitem start` 启动工作项的 Agent 会话时，按以下优先级确定模型配置：
